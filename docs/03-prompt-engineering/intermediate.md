...
CoT 是一条线性推理链，而 Tree-of-Thought（ToT）在每个步骤探索多个候选方向，形成树状搜索。模型在每一步评估各分支的前景，剪枝不靠谱的方向。
...
|------|---------|-----------|------|
| Zero-shot | 简单任务 | 1 | 低 |
| Few-shot | 格式/模式敏感 | 1 | 低 |
| Zero-shot CoT | 需要推理 | 1 | 中 |
| Manual CoT | 有推理模板 | 1 | 中 |
| Self-Consistency | 高准确性推理 | N | 高 |
| Tree-of-Thought | 开放性复杂问题 | N * depth | 很高 |

Agent 开发中最常用的组合是 **Few-shot + CoT**：用示例教模型格式，用 CoT 引导推理。
:::
...
Claude 对 XML 标签有天然的良好支持。XML 在混合文本和结构化数据的场景中特别好用：
