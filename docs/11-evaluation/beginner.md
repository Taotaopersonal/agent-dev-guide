...
class OfflineEvaluator {
  ...
  constructor(agentFn: (question: string) => Promise<string>) {
    this.agentFn = agentFn;
  }

  async evaluate(testCases: TestCase[]): Promise<Record<string, unknown>> {
    /** 批量评估 */
    ...
    for (let i = 0; i < testCases.length; i++) {
      const cas = testCases[i];
      const question = cas.question;
      const expected = cas.expected_answer;
      const tags = cas.tags ?? [];

      try {
        const actual = await this.agentFn(question);
        const isCorrect = this.checkAnswer(actual, expected);
        if (isCorrect) {
          correct++;
        } else {
          errors.push({
        ...
        this.results.push({ question, correct: isCorrect, tags });
      } catch (e) {
        errors.push({ question, error: String(e) });
        this.results.push({ question, correct: false, tags });
      }
  ...
  private checkAnswer(actual: string, expected: string): boolean {
    /** 简单的关键词匹配检查 */
...
class OnlineMonitor {
  ...
  trackFeedback(positive: boolean): void {
    /** 记录用户反馈 */
    if (positive) {
      this.metrics.user_feedback_positive += 1;
    } else {
      this.metrics.user_feedback_negative += 1;
    }
  }

  getDashboard(): Record<string, number> {
    /** 获取监控面板数据 */
    const total = this.metrics.total_requests;
    const pos = this.metrics.user_feedback_positive;
    const neg = this.metrics.user_feedback_negative;
    return {
      total_requests: total,
      avg_latency_ms: this.metrics.total_latency / Math.max(total, 1),
      avg_tokens: this.metrics.total_tokens / Math.max(total, 1),
      error_rate: this.metrics.error_count / Math.max(total, 1),
      satisfaction_rate: pos / Math.max(pos + neg, 1),
    };
  }
}
```
