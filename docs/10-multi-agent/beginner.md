...
**学完你能做到：** 判断某个任务是否需要多 Agent，选择合适的架构模式，并用 TypeScript 实现 Supervisor 和 Pipeline 两种最常用的模式。
:::
...
class SupervisorSystem {
  ...
  /** 注册一个 Worker Agent */
  addWorker(name: string, systemPrompt: string): void {
    this.workers[name] = systemPrompt;
  }
  ...
  async run(task: string): Promise<string> {
    ...
    const response = await client.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `作为 Supervisor，分析以下任务并分配给合适的 Worker。
...
返回 JSON：{"assignments": [{"worker": "名称", "subtask": "子任务描述"}]}`,
    ...
    const assignments: { worker: string; subtask: string }[] = JSON.parse(
    ...
    for (const assignment of assignments) {
      const { worker: workerName, subtask } = assignment;
      if (workerName in this.workers) {
        const result = await client.messages.create({
          model: "claude-sonnet-4-20250514",
          max_tokens: 1024,
          system: this.workers[workerName],
      ...
      messages: [
        {
          role: "user",
          content:
            `作为 Supervisor，整合以下 Worker 结果，生成最终报告。\n\n` +
            `原始任务：${task}\n\nWorker 结果：\n${resultsText}`,
        },
      ],
    });
...
// 使用示例
const system = new SupervisorSystem();
system.addWorker("researcher", "你是技术研究员，擅长收集和整理信息。");
system.addWorker("analyst", "你是数据分析师，擅长从数据中提取趋势和洞察。");
system.addWorker("writer", "你是技术写作者，擅长将复杂信息转化为清晰的报告。");

const result = await system.run("分析 2024 年 AI Agent 的发展趋势");
console.log(result);
```
  ...
  async debate(
    ...
    for (let roundNum = 0; roundNum < rounds; roundNum++) {
      for (const agent of agents) {
        // 收集其他人的观点作为上下文
        const otherViews = agents
          .filter((a) => a !== agent && a.history.length > 0)
          .map((a) => `[${a.perspective}]: ${a.history[a.history.length - 1]}`)
          .join("\n");

        const prompt = `你代表 "${agent.perspective}" 的立场。

问题：${question}
${otherViews ? "其他参与者的观点：" + otherViews : "请先发表你的观点。"}

第 ${roundNum + 1} 轮：请发表你的论点。如果有其他观点，请回应。`;
...
class PipelineSystem {
  private stages: { name: string; prompt: string }[] = [];

  /** 添加一个流水线阶段 */
  addStage(name: string, systemPrompt: string): void {
    this.stages.push({ name, prompt: systemPrompt });
  }
...
pipeline.addStage("需求分析", "你是需求分析师，提取核心需求并输出结构化需求文档。");
pipeline.addStage("架构设计", "你是架构师，基于需求设计系统架构。");
pipeline.addStage("实现方案", "你是开发者，基于架构输出详细的实现方案。");
pipeline.addStage("评审", "你是 Tech Lead，审核方案并提出改进建议。");

const result = await pipeline.run("开发一个支持多轮对话的客服机器人");
console.log(result);
```
